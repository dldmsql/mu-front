<template>
  <div class="content">
    <h1>카테고리 관리</h1>
    <button @click="openCreateModal">새 카테고리 생성</button>
    <table>
      <thead>
      <tr>
        <th>ID</th>
    ...
    <div v-if="showModal" class="modal">
      <div class="modal-content">
        ...
        <h2>{{ isEditing ? '카테고리 수정' : '새 카테고리 생성' }}</h2>
        <div>
...
export default {
  ...
  methods: {
    ...
    createItem() {
      axios.post(`${BASE_URL}/api/v1/categories`, this.form)
          .then(() => {
    ...
    updateItem() {
      axios.put(`${BASE_URL}/api/v1/categories/${this.form.id}`, this.form)
          .then(() => {
            this.fetchData(this.currentPage)
            this.closeModal()
          }).catch(error => {
    ...
    deleteItem(id) {
      axios.delete(`${BASE_URL}/api/v1/categories/${id}`)
          .then(() => {
            this.fetchData(this.currentPage)
          }).catch(error => {
